...
3. **Authenticate** (choose one method):
   - **Command line**:** `python evaluation/run_benchmark.py --benchmark HarmBench --hf-token YOUR_TOKEN`
...
python evaluation/run_benchmark.py --benchmark HarmBench --limit 100 --hf-token YOUR_TOKEN
...
python evaluation/run_benchmark.py --benchmark HarmBench --config contextual --hf-token YOUR_TOKEN

# Run JailbreakBench with custom threshold (no auth needed)
python evaluation/run_benchmark.py --benchmark JailbreakBench --threshold 0.8

# Run WildGuardMix with all examples (no auth needed)
python evaluation/run_benchmark.py --benchmark WildGuardMix
```
...
python evaluation/run_benchmark.py --all --threshold 0.75
```
...
3. Test with: `python evaluation/run_benchmark.py --benchmark NewBenchmark --limit 10`
