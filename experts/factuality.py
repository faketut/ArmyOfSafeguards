"""Factuality safeguard expert powered by DeBERTa-v3."""
...
DEFAULT_MODEL_ID = "ajith-bondili/deberta-v3-factuality-small"
...
def _model_id_from_env() -> str:
    ...
    Override model id/path via env:
      - AOS_FACTUALITY_MODEL (preferred)
      - AOS_FACTUALITY_MODEL_ID (legacy)
    """
    v = os.environ.get("AOS_FACTUALITY_MODEL", "").strip() or os.environ.get(
        "AOS_FACTUALITY_MODEL_ID", ""
    ).strip()
...
def predict(text: str) -> Dict[str, float]:
    """Run the factuality expert on *text* and return the label + confidence."""
...
def aggregate(predictions: Iterable[Mapping[str, float]]) -> Dict[str, float]:
    """Majority-vote aggregation across factuality experts."""
    votes = Counter()
...
def _build_cli() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the factuality safeguard expert")
