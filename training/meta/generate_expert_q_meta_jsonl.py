#!/usr/bin/env python3
"""
Build training rows: expert Q-values + binary label (heuristic, not dataset-native).
...
For dataset-native labels for meta, use build_meta_from_hf_labels.py instead.
"""
...
# Repo root (this file lives in training/meta/)
...
def main() -> int:
    ...
    ap.add_argument(
        "--dry-run",
        action="store_true",
        help="Deprecated: ignored (labels always use expert-Q heuristic)",
    )
    ap.add_argument(
    ...
    if args.dry_run:
        print("warning: --dry-run is deprecated and ignored.", file=sys.stderr)
    ...
    for row in rows_buffer:
        ...
        if f_m:
            meta_row = {
                ...
                "individual_results": individual_results,
                "label_source": label_provenance["label_source"],
                "label_threshold": label_provenance["label_threshold"],
                "max_expert_q": label_provenance["max_expert_q"],
            }
    ...
    if args.require_expert_outputs and processed == 0:
        print(
            "error: no rows written — all rows skipped (experts missing or failed, or empty input). "
            "Check expert model downloads (HF), GPU/CPU, and network.",
            file=sys.stderr,
        )
        return 2
    return 0
